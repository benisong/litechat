...
8. scenario 建议写到 140 到 240 个中文字符，既要交代 {{char}} 眼下所处环境，也要让人物背景和世界观自然落地；同时要写清 {{char}} 与 {{user}} 此刻的关系现状和接下来要发生的事；且应聚焦在一个正在发生或即将爆发的具体事件上，而不是静态地描述环境与关系，不能只写两人初次相遇。
9. first_msg 建议写到 120 到 260 个中文字符，必须从一个正在发生的具体情境或事件切入，{{char}} 带着明确的情绪和目的进入画面，要有动作、语气、氛围和角色感，不要像说明书，也不要从陌生人自我介绍开场（关系现状应延续 scenario 的设定），并在结尾把行动权交给 {{user}}。
...
剧情张力（重点，直接决定角色卡是否好玩）：
25. scenario 和 first_msg 的核心任务不是”交代背景”，而是让玩家一进来就被卷进一个正在发生的事。默认不要从”日常打招呼””第一次见面自我介绍””安静的下午”这种零张力状态开场。
26. 开场必须落在一个”事件临界点”上：一件刚刚发生、正在发生、或即将爆发的事——一次冲突、一个抉择、一场误会、一次摊牌、一个被撞破的秘密、一条突如其来的消息、一个不得不做的决定。{{char}} 在 first_msg 里应当是带着情绪和动机在行动，而不是站在原地等 {{user}} 开口。
27. 这个开场事件必须自然源于选定的场景、基调和关系，把人物背景里的张力点直接引爆，而不是凭空制造狗血。事件要给 {{user}} 一个明确的、立刻能接的反应入口（要回应的问题、要做的选择、要面对的人）。
28. scenario 里要埋至少一个尚未解开的钩子：一个没说出口的秘密、一个未兑现的承诺、一个被刻意回避的话题、一段没讲完的过去——让玩家想追问、想挖下去。
29. first_msg 收尾要把”球”踢给 {{user}}：停在一个需要 {{user}} 接话或行动的瞬间（一句逼问、一个动作的中途、一个递到面前的选择），而不是 {{char}} 自说自话地说完一整段就结束。
...
- first_msg 是否从一个正在发生的事件切入、并在结尾把行动权交给了 {{user}}，而不是平淡的日常开场？`)
