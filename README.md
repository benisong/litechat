...
生成提示词强化了剧情张力：开场（scenario / first_msg）聚焦于一个正在发生或即将爆发的事件，而非平淡的日常寒暄，并在结尾把行动权交给用户、埋下可追问的钩子。
...
- 支持聊天预设管理
- 支持世界书与关键词触发注入
- 支持全局世界书（不绑角色，对所有角色/会话恒定生效）与角色绑定世界书
- 创建全局世界书时可勾选「AI 文字问题修正」（默认勾选），自动加入一条全局生效的纠错条目，约束套路化身体/表情描写、烂俗比喻与生理夸张；指令逻辑用英文以增强约束力，生成后可在条目中自行修改
- 新建世界书条目时提供条目模板选择：AI 文字问题修正（注入 d0 强约束）、以及村上春树 / 古龙 / 网文女频 / 网文男频等文风模板（注入 d4 软引导，仅作风格倾向，不抢系统层注意力）；选中即生成标准条目，可自行修改，也可新建空白条目
- 聊天生成可结合预设和世界书
- 角色卡生成链路与聊天预设完全独立，避免生成草稿时被聊天 Prompt 污染
- 管理员预设页新增“记忆存储”选项卡
- “记忆存储”用于配置 service 模式自动摘要的补充提示词
- 管理员可以让记忆存储摘要跟随默认模型，或单独指定摘要模型
...
自动摘要的工作方式：
- 管理员可在“预设 / 记忆存储”中设置摘要触发字数上限，默认 `3000` 字
- 后台统计“最新安全摘要之后的未摘要有效文本”字数，并维护“是否需要摘要”和“下一次允许摘要的楼层”两个参数
- 只有字数达到上限且当前助手楼层达到目标楼层时，才记录最近一条助手消息的序号，不立即请求摘要
- 用户发送下一条消息时，先保存用户消息，再异步生成截至该序号的滚动摘要；聊天回复不会等待摘要
- 每次摘要任务被领取后，无论成功或失败，下一次允许摘要的楼层都会推进 `10` 层，避免连续重试
- 首次摘要请求携带完整的角色卡业务字段作为理解参考，以及消息序号 `1` 到本次截止序号的聊天记录
- 第二次及以后只携带排除场景设定和开场白后的角色卡参考、上一份成功摘要，以及上一截止序号之后到本次截止序号的聊天记录
- 角色卡只用于帮助摘要模型理解人物、称呼、视角和设定边界，系统明确禁止把静态角色卡资料复制进摘要结果
- 聊天上下文始终使用“最新成功摘要 + 摘要截止序号之后的原始消息”
- 从未成功摘要时继续发送完整历史；超过 `100` 条消息仍未摘要会向用户提示
- 摘要失败时继续沿用上一份成功摘要，达到下一次允许楼层后再自动重试
- 普通发送、重试、上下文组装和警告读取只访问原子内存快照；摘要条件判断、数据库读写和模型请求全部在后台执行
- 摘要状态与内容存储在独立的 `litechat-summary.db`，后台摘要写锁不会占用聊天主库；旧版本摘要会在首次启动时一次性迁移
- 摘要网络请求期间不持有数据库事务，不会阻塞聊天、删除消息或删除对话
- 用户主动删除或级联删除消息时，会在短事务中物理删除所有覆盖该序号的摘要，并恢复最近一份安全摘要
- 重新生成会先隔离该对话的摘要快照并使用完整历史，摘要失效与回退重算在后台完成，不阻塞重新生成
- 助手回复中的状态栏会原子拆分到独立 `message_status_bars` 表，`messages.content`、摘要请求和聊天历史均不再包含状态栏
- 下一轮聊天只注入最新一份状态栏作为独立系统参考，不累计历史状态栏；旧数据库中的合并状态栏会在启动时自动迁移
