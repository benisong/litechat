...
- 支持模板化创建角色卡
- 角色卡、世界书、预设提示词等多行配置支持一键放大编辑，并实时同步回原输入框
...
`性别选择 -> 场景/关系基调/性格选择预设或自定义 -> 叙事视角 -> 调用角色卡专用 Prompt -> 后端提取字段 -> 回填到角色卡编辑页`
...
- 预设和自定义内容互斥，避免模型同时收到冲突的人设方向

生成提示词强调人物自主性、关系边界和可持续互动；开场可以紧张，也可以日常、尴尬或安静，不强制制造危机或悬念。
